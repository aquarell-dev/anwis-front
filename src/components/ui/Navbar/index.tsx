...
const Navbar: FC = () => {
  ...
  const { currentSelectedIndividual } = useTypedSelector(
    ...
    <>
      <div className='h-16 bg-indigo-600 shadow-md py-2 px-4 flex items-center justify-between text-white'>
        <div className='flex items-end space-x-4'>
          <h1 className='font-medium text-xl lg:text-2xl w-full'>
            <Link to='/'>Anwis Dashboard</Link>
          </h1>
          <div className='invisible lg:visible flex items-center space-x-2 hover:underline hover:text-gray-100 transition duration-300'>
            {links.map((link) => (
              <Link
                to={link.link}
                key={link.id}
